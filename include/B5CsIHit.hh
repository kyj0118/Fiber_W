...
//
/// \file B5CsIHit.hh
/// \brief Definition of the B5CsIHit class
...
class B5CsIHit : public G4VHit
{
public:
  B5CsIHit();
  B5CsIHit(G4int cellID);
  B5CsIHit(const B5CsIHit &right);
  virtual ~B5CsIHit();
...
using B5CsIHitsCollection = G4THitsCollection<B5CsIHit>;

extern G4ThreadLocal G4Allocator<B5CsIHit>* B5CsIHitAllocator;

inline void* B5CsIHit::operator new(size_t)
{
  if (!B5CsIHitAllocator) {
       B5CsIHitAllocator = new G4Allocator<B5CsIHit>;
  }
  return (void*)B5CsIHitAllocator->MallocSingle();
}

inline void B5CsIHit::operator delete(void* aHit)
{
  B5CsIHitAllocator->FreeSingle((B5CsIHit*) aHit);
}
